...
def wait_for_central_fixation(win, el_tracker, screen_width, screen_height, px_per_deg,
                              deg_thresh=2.499, hold_ms=200,
                              cross_height=40, cross_color='black',
                              max_wait_s=None, bg=None):
    """
    Blocks the next trial from starting until gaze is within `deg_thresh` of screen
    center for >= `hold_ms`. Returns (ok, drift_deg). If `max_wait_s` is None, waits
    indefinitely. If no tracker, shows the cross for 0.5s and returns (True, "").
    """
...
def run_dynamic_visibility_trials(
    ...
    with open(filename, 'w', newline='') as csvfile:
        ...
        for i, tr in enumerate(trials, start=1):
            ...
            gate_ok, _ = wait_for_central_fixation(
                win, el_tracker, screen_width_pix, screen_height_pix, px_per_deg,
                deg_thresh=2.499, hold_ms=200,
                cross_height=40, cross_color='black',
                max_wait_s=None, bg=static_noise_stim
            )
            if not gate_ok:
                print(f"[FIXGATE] Trial {i}: timed out waiting for central fixation.")
                return filename
            # ======================================================
